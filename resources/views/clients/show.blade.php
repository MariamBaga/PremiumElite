        ...
        <div class="col-lg-8">
            <div class="card mb-3">
                ...
                <div class="card-body">
                    <dl class="row mb-0">
                        ...
                        <dd class="col-sm-9 text-nowrap">{{ optional($client->date_paiement)->format('d/m/Y') ?? '-' }}
                        </dd>

                        <dt class="col-sm-3">Date d’affectation</dt>
                        <dd class="col-sm-9 text-nowrap">{{ optional($client->date_affectation)->format('d/m/Y') ?? '-' }}
                        </dd>
            ...
            <div class="card mb-3">
                <div class="card-header">
                    Dossiers associés
                    <span
                        class="badge bg-primary ms-1">{{ $client->dossiers_count ?? $client->dossiers()->count() }}</span>
                </div>
                            ...
                            <table class="table table-hover mb-0">
                                ...
                                <tbody>
                                    @foreach ($dossiers as $d)
                                        <tr>
                                            <td>{{ $d->reference }}</td>
                                            <td>{{ \Illuminate\Support\Str::headline($d->statut?->value ?? $d->statut) }}
                                            </td>
                                            <td class="text-nowrap">{{ ucfirst($d->type_service) }}</td>
                                            <td class="text-nowrap">{{ optional($d->date_planifiee)->format('d/m/Y H:i') }}
                                            </td>
                                            <td class="text-end">
                                            @can('dossiers.view')
        ...
        <div class="col-lg-4">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    @endcan
                    <a href="tel:{{ $client->telephone }}" class="btn btn-outline-secondary w-100 mb-2"
                        @disabled(empty($client->telephone))>
                        Appeler le client
                    </a>
                    <a href="mailto:{{ $client->email }}" class="btn btn-outline-secondary w-100"
